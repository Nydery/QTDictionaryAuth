...
Das Projekt ***QTDictionaryAuth*** ist eine Vorlage fuer die Erstellung von datenzentrierten Anwendungen. Ausgehend von dieser Vorlage  
...
4. [Verwendung der Vorlage](#verwendung-der-Vorlage)  
   ...
   2. [Abgleich mit dem QTDictionaryAuth](#abgleich-mit-dem-QTDictionaryAuth)  
...
## Template  
Die Struktur vom 'QTDictionaryAuth' besteht aus unterschiedlichen Teilprojekten und diese in einer Gesamtloesung (im Kontext von Visual Studio ist das eine Solution) zusammengefasst. Eine Erlaeuterung der einzelnen Projekte, deren Typ und die Abhaengigkeit finden sie in der folgenden Tabelle:  
...
|**QTDictionaryAuth.Logic**|Dieses Projekt beinhaltet den vollstaendigen Datenzugriff, die gesamte Geschaeftslogik und stellt somit den zentralen Baustein des Systems dar.|Library|CommonBase|  
|**QTDictionaryAuth.WebApi**|In diesem Projekt ist die REST-Schnittstelle implementiert. Diese Modul stellt eine API (Aplication Programming Interface) fuer den Zugriff auf das System ueber das Netzwerk zur Verfuegung.|Host|CommonBase, QTDictionaryAuth.Logic|  
|**QTDictionaryAuth.ConApp**|Dieses Projekt dient als Initial-Anwendung zum Erstellen der Datenbank, das Anlegen von Anmeldedaten falls die Authentifizierung aktiv ist und zum Importieren von bestehenden Daten. Nach der Initialisierung wird diese Anwendung kaum verwendet.|Console|CommonBase, QTDictionaryAuth.Logic|  
|**QTDictionaryAuth.AspMvc**|Diese Projekt beinhaltet die Basisfunktionen fuer eine AspWeb-Anwendung und kann als Vorlage fuer die Entwicklung einer einer AspWeb-Anwendung mit dem QTDictionaryAuth verwendet werden.|Host|CommonBase, QTDictionaryAuth.Logic|  
|**QTDictionaryAuth.WpfApp**|Diese Projekt beinhaltet die Basisfunktionen fuer eine Wpf-Anwendung und kann als Vorlage fuer die Entwicklung einer einer Wpf-Anwendung mit dem QTDictionaryAuth Framework verwendet werden.|Host|CommonBase, QTDictionaryAuth.Logic|  
|**QTDictionaryAuth.XxxYyy**|Es folgen noch weitere Vorlagen von Client-Anwendungen wie Angular, Blazor und mobile Apps. Zum jetzigen Zeitpunkt existiert nur die AspMvc-Anwendung. Die Erstellung und Beschreibung der anderen Client-Anwendungen erfolgt zu einem spaeteren Zeitpunkt.|Host|CommonBase, QTDictionaryAuth.Logic|  
...
|Projekt|Beschreibung|Typ|Abhaengigkeit  
|---|---|---|---|  
|**TemplateCopier.ConApp**|Diese Anwendung dient zum Kopieren der Vorlage ***'QTDictionaryAuth'***. Die Vorlage dient als Basis fuer viele zukuenftige Projekte und muss dementsprechend kopiert werden. Der *TemplateCopier* kopiert alle Teilprojekte in den Zielordner und fuehrt eine Umbenennung der Komponenten durch.|Console|CommonBase  
|**TemplateComparsion.ConApp**|Dieses Projekt dient zum Abgleich der Vorlage ***'QTDictionaryAuth'*** mit den bereits erstellten Domaen-Projekten. Alle Dateien mit dem Label `@CodeCopy` im Template und dem Label `@CodeCopy` im Domaen-Projekt werden abgeglichen.|Console|CommonBase  
...
Nachfolgend werden die einzelnen Schritte von der Vorlage ***QTDictionaryAuth*** bis zum konkreten Projekt ***QTMusicStoreLight*** erlaeutert. Das Projekt ist eine einfache Anwendung zur Demonstration von der Verwendung der Vorlage. Im Projekt ***QTMusicStoreLight*** werden Kuenstler (Artisten) und deren produzierten Alben verwaltet. Jedes Album hat ein Genre (Rock, Pop, Klassik usw.) zugeordnet. Ein Datenmodell ist im nachfolgendem Abschnitt definiert.  
   
## System-Erstellungs-Prozess  
  
Wenn nun ein einfacher Service oder eine Anwendung entwickelt werden soll, dann kann die Vorlage ***QTDictionaryAuth*** als Ausgangsbasis verwendet und weiterentwickelt werden. Dazu empfiehlt sich folgende Vorgangsweise:  
  
### Vorbereitungen  
  
- Erstellen eines Ordners (z.B.: Develop)  
- Herunterladen des Repositories ***QTDictionaryAuth*** vom [GitHub](<https://github.com/leoggehrer/CSSoftwareEngineering-QTDictionaryAuth>) und in einem Ordner speichern.  
  
> **ACHTUNG:** Der Solution-Ordner von der Vorlage muss ***QTDictionaryAuth*** heißen.  
...
Als Ausgangsbasis wird die Vorlage ***QTDictionaryAuth*** verwendet. Diese Vorlage wird mit Hilfe dem Hilfsprogramm ***'TemplateCopier.ConApp'*** in ein Verzeichnis eigener Wahl kopiert. In diesem Verzeichnis werden alle Projektteile (mit Ausnahme der Hilfsprogramme *TemplateCopier.ConApp* und *TemplateComparison.ConApp*) von der Vorlage kopiert und die Namen der Projekte und Komponenten werden entsprechend angepasst. Alle Projekte mit dem Prefix ***QTDictionaryAuth*** werden mit dem domainspezifischen Namen des Verzeichnisses ersetzt. Beim Kopieren der Dateien von der Vorlage werden alle Dateien mit dem Label ***@CodeCopy*** durch den Label ***@CodeCopy*** ersetzt. Diese Label werden fuer den Abgleich-Prozess verwendet.  
...
Copy 'QTDictionaryAuth' from: ...\source\repos\HtlLeo\CSSoftwareEngineering\QTDictionaryAuth  
...
**Hinweis:** Die Vorlage muss im Ordner (*QTDictionaryAuth*) gespeichert sein.  
...
Im Projekt ***QTDictionaryAuth*** sind alle Code-Teile, welche als Basis-Code in andere Projekte verwendet werden, mit einem Label ***@CodeCopy*** markiert. Dieser Label wird im Zielprojekt mit dem Label ***@CodeCopy*** ersetzt. Das hat den Vorteil, dass Aenderungen in der Vorlage auf die bereits bestehenden Projekte uebertragen werden koennen (naehere Informationen dazu gibt es spaeter).  
...
## Abgleich mit dem QTDictionaryAuth  
  
In der Software-Entwicklung gibt es immer wieder Verbesserungen und Erweiterungen. Das betrifft die Vorlage ***QTDictionaryAuth*** genauso wie alle anderen Projekte. Nun stellt sich die Frage: Wie koennen Verbesserungen und/oder Erweiterungen von der Vorlage auf die Domain-Projekte uebertragen werden? In der Vorlage sind die Quellcode-Dateien mit den Labels ***@CodeCopy*** gekennzeichnet. Beim Kopieren werden diese Labels durch den Label ***@CodeCopy*** ersetzt. Mit dem Hilfsprogramm *TemplateComparison.ConApp* werden die Dateien mit dem Label ***@CodeCopy*** und ***@CodeCopy*** abgeglichen. In der folgenden Skizze ist dieser Prozess dargestellt:  
...
Source: ...\source\repos\HtlLeo\CSSoftwareEngineering\QTDictionaryAuth\  
...
Wird nun die Option **[1 oder a]** aktiviert, dann werden alle Dateien im Projekt **QTDictionaryAuth** mit der Kennzeichnung **@BaseCopy** mit den Dateien im Projekt **QTMusicStoreLight** mit der Kennzeichnung **@CodeCopy** abgeglichen.  
